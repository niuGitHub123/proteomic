...
y_train = y_train.map({'Non_Obese': 0, 'Obese': 1})

model = sklearn.ensemble.RandomForestRegressor(n_estimators=500, max_features=5, oob_score=True, random_state=1009)
model.fit(X_train, y_train)

explainer = shap.Explainer(model)
shap_values = explainer(X)
print(shap_values.feature_importances_)
shap_values[0]
...
shap.plots.waterfall(shap_values[0]) # the firtst sample
shap.plots.bar(shap_values)
shap.plots.heatmap(shap_values)
shap.plots.force(explainer.expected_value,shap_values.values,shap_values.data)
shap.plots.force(shap_values)
